...
{
    public async Task<Result<SelecionarFaturamentosResult>> Handle(
        SelecionarFaturamentosQuery query, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                var resultadoEmCache = JsonSerializer.Deserialize<SelecionarFaturamentosResult>(jsonString);
